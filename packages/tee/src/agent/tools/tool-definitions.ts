export function buildSystemPrompt(
  ...
  let prompt = `You are "${name}.wraith" — a privacy-obsessed AI agent living inside a Trusted Execution Environment on Horizen EVM. You exist to protect your operator's financial privacy. You are not a generic assistant. You are a guardian.
...
YOUR CAPABILITIES:
- Send private ETH/ZEN/USDC payments via stealth addresses (send_payment)
...
- Fund wallet with testnet ETH (fund_wallet)
...
export const agentTools = [
  {
    functionDeclarations: [
      {
        name: 'send_payment',
        description: 'Send ETH, ZEN, or USDC privately to a .wraith name or meta-address via stealth address',
        parameters: {
          type: 'OBJECT',
          properties: {
            recipient: { type: 'STRING', description: 'Recipient .wraith name or st:eth:0x meta-address' },
            amount: { type: 'STRING', description: 'Amount to send' },
            asset: { type: 'STRING', description: "Asset: 'ETH' (default), 'ZEN', or 'USDC'" },
          },
      ...
      {
        name: 'withdraw',
        description: 'Withdraw funds from a stealth address to a destination address. If amount is omitted or "all", withdraws the maximum possible after reserving gas fees.',
        parameters: {
          type: 'OBJECT',
          properties: {
            from: { type: 'STRING', description: 'Stealth address (0x...) to withdraw from' },
            to: { type: 'STRING', description: 'Destination address (0x...)' },
            amount: { type: 'STRING', description: 'Amount in ETH to withdraw, or "all" for maximum. If omitted, withdraws all.' },
          },
          required: ['from', 'to'],
        },
      },
      {
        ...
        parameters: {
          type: 'OBJECT',
          properties: { to: { type: 'STRING', description: 'Destination address (0x...)' } },
          required: ['to'],
        },
      },
      {
      ...
      {
        name: 'fund_wallet',
        description: 'Fund wallet with testnet ETH from deployer',
        parameters: { type: 'OBJECT', properties: {}, required: [] },
      },
      {
        name: 'pay_agent',

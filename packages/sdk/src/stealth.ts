...
export function generateStealthAddress(
  ...
  const ephPubKey = secp256k1.getPublicKey(ephPrivKey, true); // compressed
  ...
  const uncompressed = stealthPubKey.toRawBytes(false); // 65 bytes: 04 || x || y
  const pubKeyNoPrefix = uncompressed.slice(1); // 64 bytes: x || y
